...
def optimize_station_placement(game_graph, max_facilities, require_atlanta=True):
    ...
    def city_assigned_to_research_station(model, city):
        return (
                sum(model.x[possible_facility, city] for possible_facility in cities) == 1
        )
    ...
    m.max_facilities = Constraint(
        expr=sum(m.y[possible_facility] for possible_facility in cities)
             <= max_facilities
    )
    ...
    m.obj = Objective(
        sense=minimize,
        expr=sum(
            nx.shortest_path_length(game_graph, source=possible_facility, target=city)
            * m.x[possible_facility, city]
            for possible_facility in cities
            for city in cities
        ),
    )
    ...
    result = Output(tuple(city for city in cities if m.y[city] == 1), m.obj())
    return result
